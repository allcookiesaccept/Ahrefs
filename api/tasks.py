...
class AhrefsTasks:
    ...
    def collect_actual_data_rating(self, urls=None) -> pd.DataFrame:
        ...
        for url in self.urls:
            ...
            domain_rating, ahrefs_rank = self.methods.parse_domain_rating_response(response.text)
    ...
    def compare_data_rating(self, urls = None):
        if not urls:
            urls_with_date = self.load_urls_for_compare().to_dict('records')

        data = []
        data_dict = {}

        columns = ["Domain", "Url", "Date", "DR", "AR",
                   "CompareDate", "DRSecondDate", "ARSecondDate"]

        today = datetime.today().strftime("%Y-%m-%d")

        for item in urls_with_date[:3]:
            ...
            if key not in data_dict:
                ...
                dr, ar = self.methods.parse_domain_rating_response(actual_response.text)
                compare_date_response = self.methods.get_domain_rating(target=url, date=date_to_compare)
                compare_dr, compare_ar = self.methods.parse_domain_rating_response(compare_date_response.text)
                data_dict[key] = {"dr": dr, "ar": ar}
                data.append([domain, url, today, dr, ar, date_to_compare, compare_dr, compare_ar])
            else:
                logger.info("Take DR data from history")
                compare_date_response = self.methods.get_domain_rating(target=url, date=date_to_compare)
                compare_dr, compare_ar = self.methods.parse_domain_rating_response(compare_date_response.text)
                data.append([domain, url, today, data_dict[key]['dr'], data_dict[key]['ar'],
                             date_to_compare, compare_dr, compare_ar])
